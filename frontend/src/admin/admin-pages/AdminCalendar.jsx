...
import AdminTaskDetails from '../admin-components/AdminTaskDetails';
import AdminTaskStatusRequests from './AdminTaskStatusRequests';
  ...
  const fetchTasks = async () => {
    try {
      ...
      if (response.data.type === 'success') {
        const tasksWithUsers = response.data.tasks.map(task => ({
          ...task,
          assigned_users: task.assigned_users || [] 
        }));
        setTasks(tasksWithUsers);
        organizeTasksForCalendar(tasksWithUsers);
      } else {
      ...
      <div className="min-w-8xl mx-auto px-4 sm:px-6 lg:px-8 py-2">
        ...
        <div className="mb-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Task Calendar</h1>
              <p className="text-gray-600 mt-1">Monitor and manage all team tasks</p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
              <button 
                onClick={openCreateTaskModal}
                className="px-3 py-1.5 bg-gradient-to-r from-blue-600 to-blue-800 text-white rounded-lg hover:from-blue-700 hover:to-blue-900 transition-all flex items-center text-sm shadow-sm"
              >
        ...
        <div className={`flex flex-col ${viewMode === 'combined' ? 'lg:flex-row' : ''} gap-6`}>
          ...
          {(viewMode === 'combined' || viewMode === 'list') && (
            <div className={`${viewMode === 'combined' ? 'lg:w-1/2' : 'w-full'} bg-white rounded-lg shadow overflow-hidden`}>
              ...
              <div className="border-b border-gray-200">
                ...
                <div className="px-4">
                  <nav className="-mb-px flex space-x-8">
                    ...
                    <button
                      className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm ${activeTab === 'requests' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}
              ...
              {/* Task List or Requests Content */}
              {activeTab === 'requests' ? (
                <AdminTaskStatusRequests currentUser={currentUser} />
              ) : loading ? (
              ...
              {/* View All Button */}
              {filteredTasks.length > 0 && activeTab !== 'requests' && (
        ...
        {showCreateTaskModal && (
          <div 
            className="fixed inset-0 z-50 overflow-y-auto bg-transparent bg-opacity-50 backdrop-blur-sm flex items-center justify-center"
            onClick={closeCreateTaskModal}
          >
            <div 
              className="relative bg-white rounded-xl shadow-xl w-full max-w-4xl mx-4 my-8"
